...
use crate::{Point, Vec3, Ray, Hittable, HitRecord, HittableList};
use std::mem;
...
impl BVH {
    fn hit_helper(&self, r: &Ray, t_min: f64, mut t_max: f64) -> HitRecord {
        if self.bounding_box().hit(r, t_min, t_max) {
            match &self.contents {
                BVHContents::Leaf(obj) => obj.hit(r, t_min, t_max),
                BVHContents::Node{left, right} => {
                    let left = left.hit_helper(r, t_min, t_max);
                    if let HitRecord::Hit{t, ..} = &left {
                        t_max = *t;
                    }
                    let right = right.hit_helper(r, t_min, t_max);
                    if let HitRecord::Miss = &left {
                        right
                    } else {
                        left
                    }
